...
class AccountPayment(models.Model):
    ...
    is_approve_person = fields.Boolean(string='Approving Person',
                                       ...
                                       readonly=True,
                                       help="Activer /désactiver la personne de "
                                            " validation.")
    ...
    def action_post(self):
        ...
        if validation:
            if self.state == (
                ...
                raise UserError(
                    _("Uniquement les payment approuvés ou brouillon pouront être postés."))
            if any(inv.state != 'posted' for inv in
                   self.reconciled_invoice_ids):
                raise ValidationError(_("Le payement ne poura pas être procédé "
                                        "car la facture n'est pas ouverte!"))
