...
class ReportPartnerLedgerUser(models.AbstractModel):
    _inherit = 'report.accounting_pdf_reports.report_partnerledger'

    def _lines(self, data, partner):
        """Override to filter by user_ids (invoice salesperson)"""
        full_account = []
        ...
        # User filter clause
        user_clause = ""
        user_ids = data['form'].get('user_ids', [])

        params = [partner.id, tuple(data['computed']['move_state']), tuple(data['computed']['account_ids'])] + query_get_data[2]

        if user_ids:
            user_clause = ' AND m.invoice_user_id IN %s '
        ...
        query = """
            ...
            WHERE "account_move_line".partner_id = %s
                ...
                AND """ + query_get_data[1] + reconcile_clause + user_clause + """
    ...
    def _get_report_values(self, docids, data=None):
        """Override to filter partners by user_ids (invoice salesperson)"""
