...
class TreasuryCashClosing(models.Model):
    ...
    # NOUVEAU : Solde réel avec synchronisation automatique
    ...
    def create(self, vals_list):
        """Override pour gérer la numérotation journalière et initialiser le solde réel"""
    ...
    def action_validate(self):
        ...
        for closing in self:
            ...
            if closing.difference != 0:
                ...
                # Créer ou assigner à une clôture ultérieure
                next_day = closing.closing_date + timedelta(days=1)
                next_closing = self.env['treasury.cash.closing'].search([
                    ('cash_id', '=', closing.cash_id.id),
                    ('closing_date', '=', next_day),
                    ('state', 'in', ['draft', 'confirmed'])
                ], limit=1)

                if next_closing:
                    # Assigner à la clôture suivante existante
                    adjustment.closing_id = next_closing.id
                    next_closing.message_post(
                        body=_("📋 Ajustement de la clôture précédente %s ajouté automatiquement") % closing.name
                    )
                else:
                    # L'ajustement reste "libre" et sera pris en compte dans une future clôture
                    adjustment.message_post(
                        body=_(
                            "⚠️ Ajustement créé suite à la clôture %s. Sera inclus dans la prochaine clôture.") % closing.name
                    )
