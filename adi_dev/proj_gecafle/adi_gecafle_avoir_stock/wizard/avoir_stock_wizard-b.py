...
class GecafleAvoirExpressWizard(models.TransientModel):
    ...
    # SUPPRESSION du champ creer_receptions_retour pour éviter la confusion
    # SUPPRESSION du champ grouper_par_br qui n'est plus nécessaire
        ...
        for line in lignes:
            if line.nombre_colis_retour > 0:
                details.append(
                    ...
                    f"{line.nombre_colis_retour} colis ({line.qte_retour:.2f} kg) à {line.prix_unitaire:.2f} DA/kg"
                )
            else:
                details.append(
                    f"- {line.produit_id.name} ({line.producteur_id.name}): "
                    f"{line.qte_retour:.2f} kg à {line.prix_unitaire:.2f} DA/kg"
                )
...
class GecafleAvoirStockWizardLine(models.TransientModel):
    ...
    def _onchange_inclure(self):
        """Initialise la quantité quand on coche la ligne"""
        if self.inclure:
