...
class GecafleAvoirClient(models.Model):
    ...
    def _generer_avoirs_producteurs(self):
        """Génère les avoirs producteurs SANS commission et SANS liaison récap"""
        self.ensure_one()

        producteurs_data = {}

        for line in self.vente_id.detail_vente_ids:
            producteur = line.producteur_id
            if producteur not in producteurs_data:
                producteurs_data[producteur] = {
                    'montant_net': 0,
                    'lignes': []
                }
            producteurs_data[producteur]['montant_net'] += line.montant_net
            producteurs_data[producteur]['lignes'].append(line)

        total_net_vente = sum(data['montant_net'] for data in producteurs_data.values())

        for producteur, data in producteurs_data.items():
            if data['montant_net'] > 0:
                ratio = data['montant_net'] / total_net_vente
                montant_avoir_producteur = self.montant_avoir * ratio
                ...
                self.env['gecafle.avoir.producteur'].create({
                    ...
                    'montant_total': montant_avoir_producteur,  # SANS commission
                    'state': 'brouillon',
                })
